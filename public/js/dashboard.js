...
// ========== GENERA REPORT PDF (NUOVA VERSIONE CON SKILL) ==========
function generaReportPDF() {
  ...
  // Grafici da canvas
  var charts = {};
  ['chart-tiporete','chart-promotore','chart-acuradi','chart-promo-trend'].forEach(function(id){
    var el = G(id);
    if(el && el.offsetHeight > 0) {
      try { charts[id.replace('chart-','')] = el.toDataURL('image/png'); } catch(e){}
    }
  });
  
  // Tabelle da DOM
  var tables = {};
  ['table-tiporete','table-promotore','table-promo-trend'].forEach(function(id){
    var el = G(id);
    if(el && el.offsetHeight > 0) {
      tables[id.replace('table-','')] = el.outerHTML;
    }
  });
  
  // Leggi cards promotori
  var cardsData = [];
  var promoGrid = G('promo-cards-grid');
  if(promoGrid) {
    var promoCards = promoGrid.querySelectorAll('.promo-card');
    promoCards.forEach(function(card, idx){
      var headerEl = card.querySelector('.promo-card-header');
      var nameEl = card.querySelector('.promo-card-name');
      var totalEl = card.querySelector('.promo-card-total');
      var sparklineEl = card.querySelector('canvas[id^="spark-"]');
      var tableEl = card.querySelector('.promo-anni-table');
      
      var cardObj = {
        name: nameEl ? nameEl.textContent : '',
        total: totalEl ? totalEl.textContent : '',
        color: headerEl ? window.getComputedStyle(headerEl).backgroundColor : '#333'
      };
      
      // KPI della card
      var kpiEls = card.querySelectorAll('.promo-kpi');
      cardObj.kpis = {};
      kpiEls.forEach(function(kpi){
        var lbl = kpi.querySelector('.promo-kpi-label');
        var val = kpi.querySelector('.promo-kpi-value');
        if(lbl && val) {
          cardObj.kpis[lbl.textContent.trim()] = val.textContent.trim();
        }
      });
      
      // Sparkline
      if(sparklineEl && sparklineEl.offsetHeight > 0) {
        try { cardObj.sparkline = sparklineEl.toDataURL('image/png'); } catch(e){}
      }
  ...
  var reportConfig = {
    ...
    // Grafici
    charts: {
      tiporete: charts.tiporete || null,
      promotore: charts.promotore || null,
      trend: charts['promo-trend'] || null
    },
    
    // Tabelle
    tables: {
      tiporete: tables.tiporete || '',
      promotore: tables.promotore || '',
      trendTable: tables['promo-trend'] || ''
    },
  ...
  report.generatePDF().catch(function(err){
